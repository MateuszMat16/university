...
def RK_method(p53=100, NDMm=100, NDMst=100, PTEN=100, hop=10, time=17280, PTEN_off=False, is_siRNA=False, DNA_damage=False, negative_positive=False):
  ...
  # zmienianie wartosci parametrow o 20%
    if negative_positive:
         for key in parameters.keys():
             parameters[key] += (parameters[key] * 0.2)
    elif negative_positive == False:
         for key in parameters.keys():
             parameters[key] -= (parameters[key] * 0.2)

   
    # testowe wypisywanie wartosci parametrow
    for key in parameters.keys():
    ...
    plt.title("komórki nowotworowe - p53 + 20%")
    plt.ylabel("Protiein value")
    plt.xlabel("Time [s]")
    plt.legend(loc="upper left")
    plt.savefig("RKIV_p53.png")
    plt.close()

    plt.plot(time, NDMm_array, label="NDMm", color="#ffff00")
    plt.ylabel("Protiein value")
    plt.title("komórki nowotworowe - NDMm + 20%")
    plt.xlabel("Time [s]")
    plt.legend(loc="upper left")
    plt.savefig("RKIV_NDMm.png")
    plt.close()


    plt.plot(time, PTEN_array, label="PTEN", color="#cc6699")
    plt.ylabel("Protiein value")
    plt.title("komórki nowotworowe - PTEN + 20%")
    plt.xlabel("Time [s]")
    plt.legend(loc="upper left")
    plt.savefig("RKIV_PTEN.png")
    plt.close()
...
def RK_method_V2(p53=100, NDMm=100, NDMst=100, PTEN=100, hop=10, time=17280, PTEN_off=False, is_siRNA=False, DNA_damage=False, negatice_positive=False):
...
with open("./input.txt", "r") as input:
    ...
    if inputs["negative_positive"] == "1":
        negative_positive = True

# wybór zmiennego lub stałego kroku
if hop_state:
    RK_method_V2(p53, NDMm, NDMst, PTEN, hop, time, PTEN_off, is_siRNA, DNA_damage, negative_positive)

else:
    RK_method(p53, NDMm, NDMst, PTEN, hop, time, PTEN_off, is_siRNA, DNA_damage, negative_positive)
    print("Done")
